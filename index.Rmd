...
2. Country population or GDP is not statistically significant predictors of billionaire net worth.
...
Just as the box plot above demonstrated, being self-made equates to an approximate $505 million decrease in net worth (as shown by the coefficient on the variable selfMadeTRUE). However, even with a generous $\alpha$ = 0.1, the p-value of 0.221 is not statistically significant, so we cannot reject the null hypothesis that there's no difference in net worth between self-made and not-self-made billionaires. 
...
To address the second hypothesis, regarding geographic origins, I examined a number of factors.
...
```{r, echo=FALSE}
# Creating top 14 country subset
top14_countries <- c("United States", "China", "India", "Germany", "United Kingdom", "Russia", "Switzerland", "Hong Kong", "Italy", "Singapore", "Brazil", "Australia", "Taiwan", "Canada")

# Creating a binary variable indicating whether the country is in the top 14
billionaires$in_top14 <- billionaires$country %in% top14_countries

# Running a regression of top 14 country residence and net worth
fit_billionaire_top14country <- lm(finalWorth ~ in_top14, data = billionaires)

modelsummary::modelsummary(fit_billionaire_top14country,
                           statistic = c("s.e. = {std.error}",
                                         "p = {p.value}"),
                           gof_map = c("nobs", "r.squared",
                                       "adj.r.squared"))
...
```{r, echo=FALSE}
# Creating top 4 country subset
top4_countries <- c("United States", "China", "India", "Germany")

# Creating a binary variable indicating whether the country is in the top 4
billionaires$in_top4 <- billionaires$country %in% top4_countries

# Running a regression of top 4 country residence and net worth
fit_billionaire_top4country <- lm(finalWorth ~ in_top4, data = billionaires)

modelsummary::modelsummary(fit_billionaire_top4country,
                           statistic = c("s.e. = {std.error}",
                                         "p = {p.value}"),
                           gof_map = c("nobs", "r.squared",
                                       "adj.r.squared"))
...
## Conclusion
One major limitation to this cross-sectional design is that it assumes that control and treatment groups are identical on average. However, it goes without saying that billionaires cannot be assumed to be representative of the world population at large. 
